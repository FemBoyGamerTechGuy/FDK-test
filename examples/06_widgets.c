/* 06_widgets.c — the Phase 6 widget catalog, live.
 *
 * The entire interface is catalog widgets inside boxes and frames:
 * Labels, Buttons, a Toggle, Checkboxes, a Radio group, a
 * ProgressBar, Separators, and a titled Frame — no widget is placed
 * by hand and no pixel is drawn by the app; layout assigns geometry
 * and the catalog paints itself. The "Apply" button grows the
 * progress bar and rewrites the status label; "Reset" clears
 * everything; every control reports into the status line.
 *
 * After the animated phase (progress sweeping up) the demo freezes
 * into a deterministic HOLD frame for the test rig. Escape or the
 * close request ends it. Needs a system TrueType font (exits with a
 * notice otherwise).
 */
...
int main(void) {
    ...
    if (font_path == NULL) {
        fprintf(stderr, "06_widgets: no system TrueType font found — "
                        "this demo needs one\n");
        return 1;
    }
    font16 = fdk_font_load(font_path, 16);
    if (font16 == NULL) {
        fprintf(stderr, "06_widgets: font load failed\n");
        return 1;
    }
    ...
    /* Drive: sweep the progress deterministically (the rig reads the
     * HOLD frame), then hold, then exit. The window paints on the
     * frame_ready cadence like every FDK example. */
    int frames = 0;
    while (!app.quit) {
    ...
    printf("06_widgets: exited cleanly after %d frames\n", frames);
